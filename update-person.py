...
# Initialize webcam
cap = cv2.VideoCapture(0)  # Change '0' to the appropriate device index if you have multiple cameras
...
while cap.isOpened():
    ...
    for person_id, (x1, y1, x2, y2, start_time) in tracked_persons.items():
        ...
        # Draw ID and timer text on the person's head with a pink background
        cvzone.putTextRect(frame, f"ID: {person_id}", (x1, y1 - 50), scale=0.8, thickness=1, offset=5, colorR=(255, 20, 147), colorT=(255, 255, 255), colorB=(255, 20, 147))  # Pink background
        cvzone.putTextRect(frame, time_str, (x1, y1 - 30), scale=0.8, thickness=1, offset=5, colorR=(255, 20, 147), colorT=(255, 255, 255), colorB=(255, 20, 147))  # Pink background
